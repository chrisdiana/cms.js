...
var CMS = (function () {
  ...
  function handleMessage$1(debug, message) {
  ...
  var FileCollection = function FileCollection(type, layout, config) {
    this.type = type;
    this.layout = layout;
    this.config = config;
    this.files = [];
  ...
  FileCollection.prototype = {
    ...
    init: function init(callback) {
      this.getFiles(function (success, error) {
        if (error) handleMessage$1(messages['DIRECTORY_ERROR']);
        this.loadFiles(function (success, error) {
          if (error) handleMessage$1(messages['GET_FILE_ERROR']);
          callback();
        }.bind(this));
      }.bind(this));
    },
     ...
     */
    getFileListUrl: function getFileListUrl(type, config) {
     ...
     */
    getFileUrl: function getFileUrl(file, mode) {
     ...
     */
    getFileElements: function getFileElements(data) {
     ...
     * @param {function} callback - Callback function
     */
    getFiles: function getFiles(callback) {
     ...
     * @param {function} callback - Callback function
     */
    loadFiles: function loadFiles(callback) {
     ...
     */
    search: function search(attribute, _search) {
     ...
     * @returns {object} File object.
     */
    getFileByPermalink: function getFileByPermalink(permalink) {
  ...
  function Instance(options) {
    ...
    function initFileCollections(config, callback) {
      ...
      types.forEach(function (type, i) {
        collections[type].init(function () {
    ...
    function init() {
      // set config
      config = Object.assign({}, defaults, options); // create message container element if debug mode is enabled
      ...
      if (config.elementId) {
        // setup container
        config.container = document.getElementById(config.elementId); // check for hash changes

        window.addEventListener('hashchange', router);

        if (config.container) {
          // setup file collections
          initFileCollections(config, function () {
            ...
            ready = true; // TODO: registerPlugins();

            config.onload();
          });
        } else {
          handleMessage$1(config.debug, messages['ELEMENT_ID_ERROR']);
        }
      } else {
        handleMessage$1(config.debug, messages['ELEMENT_ID_ERROR']);
      }
    ...
    return {
      ...
      sort: function sort(type, _sort) {
        if (this.ready) {
          ...
          handleMessage$1(messages['NOT_READY_WARNING']);
        }
      },
