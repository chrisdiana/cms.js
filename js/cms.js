...
var CMS = {
	...
	render: function (url) {
		...
		pageLoaded(); //added to trigger custom JS
	},
	...
	renderPost: function(id) {
		CMS.posts.forEach(function(post){
			if(post.id == id) {
				//This checks if the new post-full-template exists, and if it doesn't falls back to the old way.
				var templateToUse;
				if ($("#post-full-template").length == 0) {
					templateToUse = "#post-template";
				}
				else {
					templateToUse = "#post-full-template"
				}
				console.log("Template To Use:" + templateToUse)
				var tpl = $(templateToUse).html(),
					$tpl = $(tpl);
